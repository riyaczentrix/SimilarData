...
# --- Configuration ---
FILE_NAME = 'similardata.csv' # Make sure this CSV file is in the same directory as your script
...
def preprocess_text(text):
    ...
    5. Normalizes multiple spaces to single spaces.
    6. Stripping leading/trailing spaces.
    (Removed: Removing non-alphabetic characters and lowercasing as per user request)
    """
    if pd.isna(text):
        return ""
    text = str(text)
    ...
    # Removed: re.sub(r'[^a-zA-Z\s]', '', first_line_segment)
    
    # Normalize multiple spaces to single spaces, strip leading/trailing spaces
    # Removed: .lower()
    cleaned_text = re.sub(r'\s+', ' ', first_line_segment).strip()
