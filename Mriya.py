...
Original file is located at
    https://colab.research.google.com/drive/1JMbaDmyJgC6XQESMf_3U6KnHAqPq5RXG
"""
...
try:
    ...
    nltk.download('punkt_tab') # Download 'punkt_tab' as requested

# Import the email_signature_remover module from convosense_utilities
# This assumes convosense_utilities is available in the environment.
# If not, you would need to provide a mock for it as well.
try:
    from convosense_utilities import email_signature_remover
except ImportError:
    print("Warning: 'convosense_utilities.email_signature_remover' not found. "
          "Using a mock email_signature_remover for cleaning. "
          "Please ensure the library is installed if you need full functionality.", file=sys.stderr)
    class MockEmailSignatureRemover:
        def remove_sign(self, text):
            """
            A mock function to simulate email signature removal.
            This will perform a more robust regex-based signature removal if the actual library is not available.
            It tries to find the last meaningful line of the message.
            """
            ...
            reply_and_signature_block_starters = [
                ...
                r"^\s*----- Disclaimer -----",
                r"^\s*Hyper-V Environment Report:",
                r"^\s*VCenter Environment Report:",
                r"^\s*Cluster Overview \(VcenterCluster\)",
            ...
            signature_content_patterns = [
                ...
                r"^\s*Piyush Kant|Chandan Maiti|Sagar Luthra|Arvind Mishra|Suraj Kumar Padhy|Manish Rai|SANJEEV BHARTI", # Specific names
            ]
            ...
            # Remove multiple consecutive newlines and strip overall whitespace
...
def clean_decoded_body(text):
    """
    Cleans the email body by first performing initial replacements, then using the
    email_signature_remover to remove reply chains and signatures/disclaimers.
    ...
    # --- Phase 0: Initial Replacements for Newline Feature ---
    # Replace "snip" and "Disclaimer" with "Thanks and regards" (case-insensitive)
    # This ensures they are caught by the later newline insertion logic.
    ...
    # --- Phase 1: Use email_signature_remover for cleaning ---
    # This replaces the previous Talon and aggressive regex logic.
    cleaned_text = email_signature_remover.remove_sign(cleaned_text)

    # --- Add newlines after specific phrases as requested (after all redaction/removal) ---
    cleaned_text = re.sub(r'\bThanks\s+and\s+regards\b', r'Thanks and regards\n', cleaned_text, flags=re.IGNORECASE)
    cleaned_text = re.sub(r'\bRegards\b', r'Regards\n', cleaned_text, flags=re.IGNORECASE)
    cleaned_text = re.sub(r'\bThanks\b', r'Thanks\n', cleaned_text, flags=re.IGNORECASE)
...
def consolidate_tickets(file_path, similarity_threshold=80, cosine_similarity_threshold=0.7):
    ...
    for group in groups:
        ...
        # Collect ALL original DecodedBody entries from the group_df, without filtering by meaningful dockets.
        # This ensures every original body from every row in the group is listed.
        all_original_bodies_from_group = []
        for index, row in group_filled.iterrows():
            original_body = str(row['DecodedBody']).strip()
            # Only add the body if its associated docket(s) are considered meaningful (part of the final output dockets).
            row_dockets_str = str(row['docket_no'])
            row_dockets = [d.strip() for d in row_dockets_str.split(';') if d.strip()]

            if original_body and any(docket in dockets_with_meaningful_body for docket in row_dockets):
                all_original_bodies_from_group.append(original_body)

        # Now, format this list. format_simple_numbered_list will ensure no deduplication.
        consolidated_row_data['DecodedBody'] = format_simple_numbered_list(all_original_bodies_from_group)

        # --- Consolidate DecodedBody_cleaned (using unique cleaned bodies) ---
        # Use the 'DecodedBody_cleaned' column, which is already present in group_filled
        unique_cleaned_bodies = group_filled['DecodedBody_cleaned'].unique()
        cleaned_body_for_group = "\n\n---\n\n".join(filter(None, unique_cleaned_bodies))
        consolidated_row_data['DecodedBody_cleaned'] = cleaned_body_for_group
...
# --- Test Block for email_signature_remover.remove_sign ---
print("\n--- Testing email_signature_remover.remove_sign with provided example ---")
email_message_test = '''
...
'''
cleaned_text_test = email_signature_remover.remove_sign(email_message_test)
print("Input Email:")
print(email_message_test)
print("\nCleaned Text (using email_signature_remover.remove_sign):")
